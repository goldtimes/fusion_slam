...
namespace slam {
bool StateicImuInit::AddMeasurements(const MeasureGroup& measures) {
...
bool StateicImuInit::TryInit() {
    ...
    math::ComputeMeanAndCovDiag(init_imus_, mean_gyro, cov_gyro_, [](const IMUData& data) { return data.gyro_; });
    math::ComputeMeanAndCovDiag(init_imus_, mean_acce, cov_acc_, [](const IMUData& data) { return data.acc_; });
    ...
    math::ComputeMeanAndCovDiag(init_imus_, mean_acce, cov_acc_,
...
void StateicImuInit::grad_schmit(const Eigen::Vector3d& gravity_inI, Eigen::Matrix3d& R_GtoI) {
    // 归一化z轴的方向
