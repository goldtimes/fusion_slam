...
PointCloudPtr LidarProcess::ConvertMessageToCloud(const sensor_msgs::PointCloud2::ConstPtr& lidar_msg) {
    if (LidarModel::GetInstance()->lidar_sensor_type_ == LidarModel::LIDAR_TYPE::Velodyne) {
        ...
        std::for_each(std::execution::par, indices.begin(), indices.end(), [&](const size_t& index) {
            ...
            // to us
            point.time = static_cast<float>(cloud_vel.points[index].time *
        ...
        std::for_each(std::execution::par, indices.begin(), indices.end(), [&](const size_t& index) {
            ...
            // to us
            point.time = static_cast<float>(cloud_ouster.points[index].t *
                                            SystemConfig::GetInstance().lidar_config.lidar_time_scale);
            cloud_ptr->points[index] = point;
        });
        ...
        std::for_each(std::execution::par, indices.begin(), indices.end(), [&](const size_t& index) {
            ...
            // to us
            point.time = static_cast<float>((cloud_rs.points[index].timestamp - cloud_rs.points[0].timestamp) *
                                            SystemConfig::GetInstance().lidar_config.lidar_time_scale);
            cloud_ptr->points[index] = point;
        });
        ...
        std::for_each(std::execution::par, indices.begin(), indices.end(), [&](const size_t& index) {
            ...
            // to cloud_ls
            point.time = static_cast<float>(cloud_ls.points[index].timestamp *
                                            SystemConfig::GetInstance().lidar_config.lidar_time_scale);
            cloud_ptr->points[index] = point;
        });
