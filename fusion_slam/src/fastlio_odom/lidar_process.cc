...
FastLidarProcess::FastLidarProcess(const LIONodeConfig &config, std::shared_ptr<FastlioIESKF> kf)
    : m_config(config), m_kf(kf) {
    m_ikdtree = std::make_shared<KD_TREE<PointType>>();
    m_ikdtree->set_downsample_param(m_config.map_resolution);
    m_cloud_down_lidar.reset(new PointCloud);
    m_cloud_down_world.reset(new PointCloud(10000, 1));
    m_norm_vec.reset(new PointCloud(10000, 1));
    m_effect_cloud_lidar.reset(new PointCloud(10000, 1));
    ...
    m_kf->SetLossFunc([&](NavState &s, SharedState &d) { updateLossFunc(s, d); });
    m_kf->SetStopFunc([&](const V21D &delta) -> bool {
        Vec3d rot_delta = delta.block<3, 1>(0, 0);
        Vec3d t_delta = delta.block<3, 1>(3, 0);
        return (rot_delta.norm() * 57.3 < 0.01) && (t_delta.norm() * 100 < 0.015);
    });
}

void FastLidarProcess::trimCloudMap() {
    m_local_map.cub_to_rm.clear();
    const NavState &state = m_kf->GetState();
    Eigen::Vector3d pos_lidar = state.P_ + state.R_ * state.t_LinI;
    ...
    float mov_dist = std::max((m_config.cube_len - 2.0 * m_config.move_thresh * m_config.det_range) * 0.5 * 0.9,
...
void FastLidarProcess::incrCloudMap() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        point = state.R_ * (state.R_LtoI * point + state.t_LinI) + state.P_;
        ...
        mid_point.x = std::floor(m_cloud_down_world->points[i].x / m_config.map_resolution) * m_config.map_resolution +
                      0.5 * m_config.map_resolution;
        mid_point.y = std::floor(m_cloud_down_world->points[i].y / m_config.map_resolution) * m_config.map_resolution +
                      0.5 * m_config.map_resolution;
        mid_point.z = std::floor(m_cloud_down_world->points[i].z / m_config.map_resolution) * m_config.map_resolution +
                      0.5 * m_config.map_resolution;

        // 如果该点所在的voxel没有点，则直接加入地图，且不需要降采样
        if (fabs(points_near[0].x - mid_point.x) > 0.5 * m_config.map_resolution &&
...
void FastLidarProcess::initCloudMap(PointVec &point_vec) {
    m_ikdtree->Build(point_vec);
}

void FastLidarProcess::process(MeasureGroup &package) {
    ...
    //                         return (rot_delta.norm() * 57.3 < 0.01) && (t_delta.norm() * 100 < 0.015); });
    if (m_config.scan_resolution > 0.0) {
        m_scan_filter.setInputCloud(package.curr_cloud);
        m_scan_filter.filter(*m_cloud_down_lidar);
    } else {
        pcl::copyPointCloud(*package.curr_cloud, *m_cloud_down_lidar);
    }
    LOG_INFO("current size:{}", m_cloud_down_lidar->size());
    trimCloudMap();
    m_kf->Update();
    incrCloudMap();
}

void FastLidarProcess::updateLossFunc(NavState &state, SharedState &share_data) {
    int size = m_cloud_down_lidar->size();
#ifdef MP_EN
...
#pragma omp parallel for
    ...
    for (int i = 0; i < size; i++) {
        ...
        Eigen::Vector3d point_world_vec = state.R_ * (state.R_LtoI * point_body_vec + state.t_LinI) + state.P_;
    ...
    for (int i = 0; i < effect_feat_num; i++) {
        ...
        Eigen::Matrix<double, 1, 3> B =
            -norm_vec.transpose() * state.R_ * Sophus::SO3d::hat(state.R_LtoI * laser_p_vec + state.t_LinI);
        J.block<1, 3>(0, 0) = B;
        J.block<1, 3>(0, 3) = norm_vec.transpose();
        if (m_config.esti_il) {
            Eigen::Matrix<double, 1, 3> C =
                -norm_vec.transpose() * state.R_ * state.R_LtoI * Sophus::SO3d::hat(laser_p_vec);
            Eigen::Matrix<double, 1, 3> D = norm_vec.transpose() * state.R_;
            J.block<1, 3>(0, 6) = C;
            J.block<1, 3>(0, 9) = D;
        }
